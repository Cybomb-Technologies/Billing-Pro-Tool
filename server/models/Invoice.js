...
const invoiceItemSchema = new mongoose.Schema({
  ...
  discount: {
    // Added Discount (percent or fixed value, let's assume value or percent. Usually value is safer for totals, but UI might use percent. Let's add both or just value. Let's add value for now)
    type: Number,
    default: 0,
    min: 0,
  },
...
invoiceSchema.pre("save", function (next) {
  ...
  if (this.taxDetails.gstType === "cgst_sgst") {
    ...
    const avgRate =
      this.subtotal > 0 ? (calculatedTotalTax / this.subtotal) * 100 : 0;
  ...
  if (this.isNew && !this.invoiceNumber) {
    try {
      ...
      if (lastInvoice && lastInvoice.invoiceNumber) {
        const lastSeqStr = lastInvoice.invoiceNumber.substring(8); // Get the part after the date
